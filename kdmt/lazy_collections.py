...
class AbstractLazySequence:
    ...
    def __lt__(self, other):
        if type(other) != type(self):
            raise f"unorderable types: %s() %s %s()".format("<", self, other)
...
class LazyMap(AbstractLazySequence):
    ...
    equivalent:
        ...
        >>> from kdmt.lazy_collections import LazyMap
        >>> function = str
        >>> sequence = [1,2,3]
...
class LazyZip(LazyMap):
    ...
    ``zip``.  In particular, an evaluated LazyZip is equivalent to a zip:
        ...
        >>> from kdmt.lazy_collections import LazyZip
...
class LazyEnumerate(LazyZip):
    ...
    equivalent:
        ...
        >>> from kdmt.lazy_collections import LazyEnumerate
...
class Trie(dict):
    ...
    def insert(self, string):
        ...
        >>> from kdmt.lazy_collections import Trie
        >>> trie = Trie(["abc", "def"])
